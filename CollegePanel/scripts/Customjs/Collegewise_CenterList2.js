...
app1.controller('examfeelistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.studentListafterfeeSubmit = function () {
        ...
        var BackStatus = $scope.BackStatus === undefined ? '' : $scope.BackStatus;
        var cCenterType = $scope.CenterType === undefined ? '' : $scope.CenterType;
        if (session == "") {
            alert('Please Select Sesstion');
            return false;
        }
        if (EducationType == "") {
            alert('Please Select Programm');
            return false;
        }
        ...
        showloader();
        $http.get(Url + "api/Collegewise_CenterList2", {
            params: {
                pageIndex: $scope.pageIndex, pageSize: $scope.pageSizeSelected, CollegeID: CollegeID, session: session, EducationType: EducationType, CourseCategoryID: CourseCategoryID, Subject: Subject, CourseYearID: CourseYearID, Application: Application, ApplicationStatus: ApplicationStatus, paymentStatus: paymentStatus, BackStatus: BackStatus, CenterType: cCenterType
            },
