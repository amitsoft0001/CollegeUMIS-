...
var app1 = angular.module('leavelistApp', ['chieffancypants.loadingBar', /*'ngAnimate',*/ 'ui.bootstrap']);
app1.config(function (cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = true;
    cfpLoadingBarProvider.latencyThreshold = 500;
});
app1.controller('leavelistCtrl', function ($scope, $http, cfpLoadingBar) {
    ...
    $scope.EmployeeLeaveList = function () {
        cfpLoadingBar.start();
       // debugger;
        var Month = $scope.Month === undefined ? '' : $scope.Month;
        var Year = $scope.Year === undefined ? '' : $scope.Year;
     
        $http.get(Url + "api/leaveDetailList", {
            params: { pageIndex: $scope.pageIndex, pageSize: $scope.pageSizeSelected, Month: Month, Year: Year },
